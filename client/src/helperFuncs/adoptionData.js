...
const fetchUsersByIds = async (userIds) => {
    try {
        const response = await axios.post('/users/all', 
            { ids: userIds },
            {
                headers: {
                    'Content-Type': 'application/json'
...
export const fetchAllData = async (petId, userIds) => {
    try {
        ...
        const [petData, usersData] = await Promise.all([petDataPromise, usersDataPromise]);
        ...
        return {
            petData,
            userData,
            organizationData: orgsData // Fallback to organizations from user data if needed
        };
    } catch (error) {
